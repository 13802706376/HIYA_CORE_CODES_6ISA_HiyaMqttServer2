...
{/*
    private static final InternalLogger logger = InternalLoggerFactory.getInstance(NettyMqttServer.class);

    private int port;
    // private final int port = ConfigService.getIntProperty("tcp.port", 1883);
    private final int httpPort = ConfigService.getIntProperty("http.port", 8080);

    private List<Channel> channels = new ArrayList<Channel>();
    private EventLoopGroup bossGroup = new NioEventLoopGroup(1);
    private EventLoopGroup workerGroup = new NioEventLoopGroup();

    public NettyMqttServer(int port)
    {
        this.port = port;
    }
    ...
    public ChannelFuture run() throws Exception
    {
        ...
        Channel channle = getDefaultServerBootstrap().childHandler(new TcpChannelInitializer()).bind(port).sync()
                .channel();
